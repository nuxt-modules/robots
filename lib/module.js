...
async function getRules (options, req = null) {
  ...
  for (const item of items) {
    const newItem = parseItem(item)
    const keys = Object.keys(correspondences).filter(key => typeof newItem[key] !== 'undefined')

    for (const key of keys) {
      let values = typeof newItem[key] === 'function' ? await newItem[key].call(this, req) : newItem[key]
...
function parseItem (item) {
  ...
  for (const [key, value] of Object.entries(item)) {
    const newkey = String(key).toLowerCase().replace('-', '').replace('_', '')
