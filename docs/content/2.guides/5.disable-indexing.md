...
The module then changes two outputs:
...
`Disallow: /` stops the crawler before the fetch.
A crawler that obeys `robots.txt` never sees the `noindex` directive.

Use `Disallow: /` for staging and preview environments that no other site links to.
Search engines seldom find these URLs.
The rule also saves crawl budget.

Do not use it for a site that search engines already know.
If another site links to a blocked URL, Google can index the URL without its content.
Search Console reports these URLs as "Indexed, though blocked by robots.txt".
Google can also keep URLs that it indexed before.
It cannot read the new `noindex` directive on them.

::note
If search engines must see the `noindex` directive of a page, do not block that page in `robots.txt`. See the [robots.txt guide](/learn-seo/nuxt/controlling-crawlers/robots-txt).
::
...
Every page gets the `noindex, nofollow` robots meta tag and `X-Robots-Tag` header.
Crawlers can fetch each page and read the directive.
If a `public/robots.txt` file disallows `/`, delete it.
