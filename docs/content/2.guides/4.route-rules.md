...
- `X-Robots-Tag` header - All modes. The module also writes the header to route rule `headers`, so static hosts send it on static files.
- `<meta name="robots">`{lang="html"}
- `/robots.txt` disallow entry - When [disallowNonIndexableRoutes](/docs/robots/api/config#disallownonindexableroutes) is enabled
