...
This blocks all bots from crawling your site.
It does not keep every URL out of search results.
If other sites link to a blocked URL, Google can still index it.
Crawlers cannot fetch the page, so they never read its `noindex` directive.
See [Disabling Site Indexing](/docs/robots/guides/disable-indexing#blocking-crawling-versus-blocking-indexing) for the trade-off.
