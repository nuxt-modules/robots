...
This includes API routes, prerendered JSON files, `/robots.txt` and build assets.
For static hosts, the module writes one `/**` header rule.
Cloudflare and Netlify read this rule from `_headers`.
The module reads the site config at build time for this rule.
If you set `NUXT_SITE_ENV` or `NUXT_SITE_INDEXABLE` only at runtime, static files and API routes keep the build value.
Set these variables at build time too.
