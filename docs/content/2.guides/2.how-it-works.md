...
Nuxt Robots tells robots (crawlers) how to behave by creating a `robots.txt` file for you, adding an `X-Robots-Tag` header and `<meta name="robots">` tag to your site
where appropriate.
...
For this reason, the module by default will provide an `X-Robots-Tag` header and `<meta name="robots">`{lang="html"} tag.
...
If the site is not indexable, every response gets the `robotsDisabledValue` header.
This includes API routes, prerendered files such as JSON, `/robots.txt` and build assets.
For static hosts, the module writes one `/**` header rule, which Cloudflare and Netlify read from `_headers`.
The module reads the site config at build time for this rule.
If you set `NUXT_SITE_ENV` or `NUXT_SITE_INDEXABLE` only at runtime, static files and API routes keep the value from the build.
Set them at build time too.
