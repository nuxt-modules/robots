...
This is called when Nitro starts (`init`) as well as when generating `robots.txt`.
...
**Per-domain indexing.** Use `ctx.event` to apply different rules based on the request hostname.
