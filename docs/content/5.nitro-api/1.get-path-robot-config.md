...
relatedPages:
  - path: /docs/robots/nitro-api/get-site-robot-config
    title: getSiteRobotConfig()
  - path: /docs/robots/api/use-robots-rule
    title: useRobotsRule()
  - path: /docs/site-config/nitro-api/get-site-indexable
...
- `options`: Optional options.
  ...
  - `path: string`{lang="ts"}: An override for which path to check. By default, it will use the current path of the `H3Event`.

### Returns

- `rule: string`{lang="ts"}: The rule for the page.
- `indexable: boolean`{lang="ts"}: Whether the page is indexable.
- `debug?: { source: string, line?: string }`{lang="ts"}: Debug information about the source of the rule and the matching line. This is only available in development mode.
