...
The [robots.txt validation](/tools/robots-txt-validator) system now supports warnings in addition to errors. The first warning checks for `Disallow: /api` rules, which may unintentionally block API routes that need to be accessible.
