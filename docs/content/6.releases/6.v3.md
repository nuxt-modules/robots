...
- Type: `{ userAgent: []; allow: []; disallow: []; comment: [] }[]`{lang="ts"}
- Default: `[]`{lang="ts"}
...
**Nuxt Site Config integration.** The module now integrates with the [nuxt-site-config](https://github.com/harlan-zw/nuxt-site-config) module.
